...
#[tokio::test]
async fn cancellation_and_deadline_before_resolution_do_not_start_a_task() {
    let cancellation = NodeCapabilityExecutionCancellation::active();
    cancellation.cancel();
    assert_interrupted(
        1,
        context(1, Instant::now() + Duration::from_secs(5), cancellation),
        NodeCapabilityExecutionFailure::Cancelled,
    )
    .await;
    assert_interrupted(
        2,
        context(2, Instant::now(), NodeCapabilityExecutionCancellation::active()),
...
async fn assert_interrupted(
    ...
    expected: NodeCapabilityExecutionFailure,
) {
    ...
    .unwrap();
    let error =
        capability.execute_node_capability(request(&capability, seed, context)).await.unwrap_err();
