...
use super::AssistantProtocolError;

pub(super) fn decode_strict_json(encoded: &[u8]) -> Result<Value, AssistantProtocolError> {
    serde_json::from_slice::<NoDuplicateValue>(encoded)
        .map(|value| value.0)
        .map_err(|_| AssistantProtocolError::InvalidJson)
}

pub(super) fn json_depth(value: &Value, depth: usize) -> usize {
    match value {
        Value::Array(values) => {
            values.iter().map(|value| json_depth(value, depth + 1)).max().unwrap_or(depth + 1)
        }
        Value::Object(values) => {
            values.values().map(|value| json_depth(value, depth + 1)).max().unwrap_or(depth + 1)
        }
        _ => depth,
    }
}
