...
crates/backends/    exact provider routers and vendor routes
...
React / Python adapter
        -> Desktop boundary and composition
             -> Project, Workflow, Asset, and Assistant use cases/interfaces

Workflow, Asset, Assistant       -> ProjectId from Project
Node Capability implementations -> WorkflowNodeCapabilityInterface
Node Capability implementations -> exact provider and managed-media interfaces
Provider routers/routes          -> exact provider interfaces
...
post-commit worker consumes only the three closed effect types defined by the backend freeze; it is
